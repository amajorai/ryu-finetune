...
# configured => reject all. Closes the unauthenticated arbitrary-path model load
# (POST /finetune/merge) and unbounded GPU-job spawn to local + cross-origin callers.
...
class LoraConfig(BaseModel):
    r: Optional[int] = Field(None, ge=1, le=256)
    alpha: Optional[int] = Field(None, ge=1, le=512)
    dropout: Optional[float] = Field(None, ge=0.0, le=1.0)
    target_modules: Optional[list[str]] = Field(None, min_length=1, max_length=32)

    @field_validator("target_modules")
    @classmethod
    def target_module_names_are_bounded(cls, values: Optional[list[str]]) -> Optional[list[str]]:
        if values is None:
            return None
        for value in values:
            if (
                ...
                raise ValueError(
                    "target_modules entries must be simple ASCII names of at most 128 characters"
                )
        return values


class TrainingConfig(BaseModel):
    epochs: Optional[float] = Field(None, gt=0.0, le=100.0)
    max_steps: Optional[int] = Field(None, ge=0, le=100_000)
    learning_rate: Optional[float] = Field(None, gt=0.0, le=1.0)
    batch_size: Optional[int] = Field(None, ge=1, le=128)
    grad_accum: Optional[int] = Field(None, ge=1, le=128)
    max_seq_length: Optional[int] = Field(None, ge=128, le=32_768)
...
class FinetuneRequest(BaseModel):
    base_model_id: str = Field(
        ...
        description="Hugging Face repo id (namespace/model; ideally an unsloth/*-bnb-4bit).",
    )
