...
async fn main() -> anyhow::Result<()> {
    ...
    let ctx = FinetuneCtx::new(store.clone(), client, unsloth_url).with_worker_token(token.clone());
    ...
    // endpoint is what makes the whole `/api/finetune` surface callable by an agent
    // (`ryu_finetune::api::openapi()` was dead code until now — only tests read it).
    //
    // Root, not under `/api/finetune`: Core tries the root FIRST and only falls back
    ...
    let finetune = Router::new()
        ...
        .route(
            "/openapi.json",
            get(|| async { Json(ryu_finetune::api::openapi()) }),
        )
...
#[cfg(test)]
mod tests {
    use super::{bearer_ok, ct_eq, health, require_finetune_token};
    use axum::{
        body::Body,
    ...
    use ryu_finetune::FinetuneStore;
    ...
    fn temp_store() -> FinetuneStore {
        ...
        let dir = std::env::temp_dir().join(format!(
            "ryu-finetune-main-{}-{nanos}-{seq}",
            std::process::id()
        ));
        FinetuneStore::open(dir.join("finetune.db")).unwrap()
    }
    ...
    #[tokio::test]
    async fn health_reports_ok_and_job_count() {
        let store = temp_store();
        let (status, v) = body_json(health(store.clone()).await).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(v["ok"], serde_json::json!(true));
        assert_eq!(v["jobCount"], serde_json::json!(0));

        // Recording a job is reflected in the next probe.
        store
            .record(&ryu_finetune::FinetuneJob {
                id: "j".into(),
                base_model: "m".into(),
                output_name: None,
                state: "queued".into(),
            ...
            .await
            .unwrap();
        let (_, v) = body_json(health(store).await).await;
        assert_eq!(v["jobCount"], serde_json::json!(1));
    }

    /// Build the middleware-gated router the sidecar wires in `main`, so the auth
    /// layer + `require_finetune_token` run for real.
    fn gated(expected: Option<String>) -> Router {
        Router::new()
    ...
    #[tokio::test]
    async fn middleware_allows_matching_bearer() {
        let resp = gated(Some("tok".into()))
            .oneshot(req(Some("tok")))
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn middleware_rejects_wrong_and_missing_bearer() {
        let app = gated(Some("tok".into()));
        let bad = app.clone().oneshot(req(Some("nope"))).await.unwrap();
        assert_eq!(bad.status(), StatusCode::UNAUTHORIZED);
        let none = app.oneshot(req(None)).await.unwrap();
        assert_eq!(none.status(), StatusCode::UNAUTHORIZED);
    }

    #[tokio::test]
    async fn middleware_is_fail_closed_without_configured_token() {
        // No expected token → every request rejected, even a plausible-looking one.
        let resp = gated(None).oneshot(req(Some("tok"))).await.unwrap();
