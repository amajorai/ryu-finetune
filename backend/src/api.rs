...
use crate::store::{
    FinetuneJob, FinetuneStore, FinetuneTenantContext, FinetuneTenantScope, StartIdempotencyClaim,
};
...
async fn dispatch_unkeyed(
    ...
    match target.as_str() {
        "remote" => return dispatch_remote(ctx, tenant, &body, base_model, idempotency_key).await,
        "local" => {}
        _ => {
            return Err((
                StatusCode::BAD_REQUEST,
