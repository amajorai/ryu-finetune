...
const SELECT_ONE_SQL: &str = "SELECT id, base_model, output_name, state, target, remote_url, remote_token, output_ref, error, created_at, updated_at
...
impl FinetuneStore {
    ...
    /// write** (`None` for an id this node never recorded).
    ///
