...
use ryu_finetune::{FinetuneJob, FinetuneStore, StartIdempotencyClaim};
use serde_json::json;
...
#[tokio::test]
async fn record_if_absent_never_overwrites_a_job_id_collision() {
    let store = FinetuneStore::open(temp_db_path()).unwrap();
    store
        .record(&job("collision", "2024-01-01T00:00:00Z"))
        .await
        .unwrap();
    let mut attacker = job("collision", "2024-01-02T00:00:00Z");
    attacker.state = "failed".to_owned();
    assert!(!store.record_if_absent(&attacker).await.unwrap());
    let retained = store.get("collision").await.unwrap().unwrap();
    assert_eq!(retained.state, "queued");
    assert_eq!(retained.updated_at, "2024-01-01T00:00:00Z");
}

#[tokio::test]
async fn stale_snapshot_cannot_regress_a_terminal_job() {
    let store = FinetuneStore::open(temp_db_path()).unwrap();
    let mut finished = job("stale", "2024-01-01T00:00:00Z");
    finished.state = "succeeded".to_owned();
    store.record(&finished).await.unwrap();

    let prior = store
        .sync_from_snapshot("stale", "running", None, None, "2024-01-02T00:00:00Z")
        .await
        .unwrap();
    assert!(
        prior.is_none(),
        "a stale snapshot must not claim a transition"
    );
    assert_eq!(
        store.get("stale").await.unwrap().unwrap().state,
        "succeeded"
    );
}

#[tokio::test]
async fn start_idempotency_claim_replays_and_rejects_key_reuse() {
    let store = FinetuneStore::open(temp_db_path()).unwrap();
    assert_eq!(
        store.claim_start("retry-1", "fingerprint-a").await.unwrap(),
    ...
    assert_eq!(
        store.claim_start("retry-1", "fingerprint-b").await.unwrap(),
        StartIdempotencyClaim::Conflict
    );

    let response = json!({ "job_id": "job-1", "state": "running" });
    assert!(store
        .complete_start("retry-1", "fingerprint-a", &response)
        .await
        .unwrap());
    assert_eq!(
        store.claim_start("retry-1", "fingerprint-a").await.unwrap(),
        StartIdempotencyClaim::Replay(response)
    );
}
...
async fn remote_token_is_persisted_but_never_serialized() {
    ...
    let store = FinetuneStore::open(db_path.clone()).unwrap();
    ...
    let raw_token: String = rusqlite::Connection::open(&db_path)
        .unwrap()
        .query_row(
            "SELECT remote_token FROM finetune_jobs WHERE id = 'remote-job'",
            [],
            |row| row.get(0),
        )
        .unwrap();
    assert!(raw_token.starts_with("enc:v1:"));
    assert!(!raw_token.contains("super-secret-bearer"));
...
async fn dispatch_remote_without_url_is_rejected_before_network() {
    let store = FinetuneStore::open(temp_db_path()).unwrap();
    let ctx = FinetuneCtx::new(store, reqwest::Client::new(), DEFAULT_UNSLOTH_URL);
    let body = json!({ "base_model_id": "org/m", "target": "remote" });
