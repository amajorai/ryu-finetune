...
async fn dispatch_remote_rejects_a_job_id_collision_without_overwriting() {
    ...
    assert_eq!(
        error["error"],
        json!("remote node returned a duplicate job_id")
    );
    assert_eq!(
        store.get("collision").await.unwrap().unwrap().state,
        "queued"
    );
}
...
async fn public_routes_are_scoped_to_owner_org_and_node() {
    ...
    store
        ...
        .await
        .unwrap();
    store.record_for(&bob, &local_job("bob-job")).await.unwrap();
    let ctx = ctx_for(store.clone(), DEAD_URL);

    let (status, body) =
        oneshot_as(routes(ctx.clone()), "GET", "/list", "alice", "org-shared").await;
