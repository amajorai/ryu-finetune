...
use ryu_finetune::api::{
    cancel_value, capability_value, dispatch, dispatch_with_idempotency, get_value, list_value,
    merge_value, routes, stream_response, tenant_from_headers, FinetuneCtx,
};
use ryu_finetune::{FinetuneJob, FinetuneStore, FinetuneTenantContext};
...
fn ctx_for_with_worker_token(
    store: FinetuneStore,
    url: impl Into<String>,
    token: &str,
) -> FinetuneCtx {
    ctx_for(store, url).with_worker_token(Some(token.to_owned()))
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn dispatch_retries_with_the_same_key_without_starting_a_second_job() {
    let hits = std::sync::Arc::new(AtomicU64::new(0));
    let route_hits = std::sync::Arc::clone(&hits);
    let node = spawn(
        Router::new().route(
            "/api/finetune/start",
            post(move |Json(_body): Json<Value>| {
                let route_hits = std::sync::Arc::clone(&route_hits);
                async move {
                    route_hits.fetch_add(1, Ordering::Relaxed);
                    Json(json!({ "job_id": "idempotent-job", "state": "running" }))
                }
            }),
        ),
    )
    ...
    let body = json!({
        "base_model_id": "org/m",
        "target": "remote",
        "remote": { "url": node, "token": "node-token" },
    });

    let first = dispatch_with_idempotency(&ctx, body.clone(), Some("retry-key"))
        .await
        .expect("first dispatch");
    let second = dispatch_with_idempotency(&ctx, body, Some("retry-key"))
        .await
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn dispatch_remote_rejects_a_job_id_collision_without_overwriting() {
    async fn start(Json(_body): Json<Value>) -> Response {
        Json(json!({ "job_id": "collision", "state": "running" })).into_response()
    }
    let node = spawn(Router::new().route("/api/finetune/start", post(start))).await;
    let store = temp_store();
    store.record(&local_job("collision")).await.unwrap();
    let ctx = ctx_for(store.clone(), DEAD_URL);
    let body = json!({
        "base_model_id": "org/m",
        "target": "remote",
    ...
    let (code, error) = dispatch(&ctx, body).await.unwrap_err();
    assert_eq!(code, StatusCode::BAD_GATEWAY);
    assert_eq!(error["error"], json!("remote node returned a duplicate job_id"));
    assert_eq!(store.get("collision").await.unwrap().unwrap().state, "queued");
}
...
async fn get_value_prefers_live_worker_snapshot() {
    ...
    .await;
    let store = temp_store();
    store.record(&local_job("job-x")).await.unwrap();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn local_worker_requests_forward_the_shared_bearer() {
    let store = temp_store();
    store.record(&local_job("auth-1")).await.unwrap();
    let worker = spawn(Router::new().route(
        "/finetune/:id",
        get(|headers: HeaderMap, Path(id): Path<String>| async move {
            ...
            {
                return (StatusCode::UNAUTHORIZED, "missing worker bearer").into_response();
            }
            Json(json!({ "id": id, "state": "running" })).into_response()
        }),
    ))
    .await;
    let ctx = ctx_for_with_worker_token(store, worker, " worker-secret ");

    let value = get_value(&ctx, "auth-1")
        .await
        .expect("authenticated worker read");
...
async fn cancel_value_local_worker_error_is_bad_gateway() {
    ...
    .await;
    let store = temp_store();
    store.record(&local_job("c2")).await.unwrap();
    let ctx = ctx_for(store, worker);
...
async fn stream_local_worker_success_is_event_stream() {
    ...
    let store = temp_store();
    store.record(&local_job("s1")).await.unwrap();
    let ctx = ctx_for(store, worker);
...
async fn stream_worker_non_success_is_bad_gateway() {
    ...
    .await;
    let store = temp_store();
    store.record(&local_job("s2")).await.unwrap();
    let ctx = ctx_for(store, worker);
...
async fn stream_worker_unreachable_is_bad_gateway() {
    let store = temp_store();
    store.record(&local_job("s3")).await.unwrap();
    let ctx = ctx_for(store, DEAD_URL);
...
async fn oneshot_as(
    ...
    let req = Request::builder()
        .method(method)
        .uri(uri)
        .header("x-ryu-caller-user-id", user)
        .header("x-ryu-caller-org-id", org)
        .body(Body::empty())
        .unwrap();
    let resp = router.oneshot(req).await.unwrap();
    let status = resp.status();
...
#[test]
fn tenant_header_context_uses_server_node_and_fails_closed_when_managed() {
    ...
    headers.insert("x-ryu-caller-user-id", "alice".parse().unwrap());
    headers.insert("x-ryu-caller-org-id", "org-1".parse().unwrap());
    headers.insert("x-ryu-node-id", "attacker-node".parse().unwrap());
    let tenant = tenant_from_headers(&headers, "server-node", true).unwrap();
...
#[tokio::test]
async fn public_routes_are_scoped_to_owner_org_and_node() {
    let store = temp_store();
    let alice = FinetuneTenantContext {
        owner_user_id: Some("alice".to_owned()),
        org_id: Some("org-shared".to_owned()),
        node_id: store.node_id().to_owned(),
    };
    let bob = FinetuneTenantContext {
        owner_user_id: Some("bob".to_owned()),
        org_id: Some("org-shared".to_owned()),
        node_id: store.node_id().to_owned(),
    };
    store
        .record_for(&alice, &local_job("alice-job"))
        .await
        .unwrap();
    store
        .record_for(&bob, &local_job("bob-job"))
        .await
        .unwrap();
    let ctx = ctx_for(store.clone(), DEAD_URL);

    let (status, body) = oneshot_as(routes(ctx.clone()), "GET", "/list", "alice", "org-shared").await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["jobs"].as_array().unwrap().len(), 1);
    assert_eq!(body["jobs"][0]["id"], "alice-job");

    let (status, body) = oneshot_as(routes(ctx.clone()), "GET", "/list", "bob", "org-shared").await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["jobs"].as_array().unwrap().len(), 1);
    assert_eq!(body["jobs"][0]["id"], "bob-job");

    let (status, _) = oneshot_as(routes(ctx.clone()), "GET", "/alice-job", "bob", "org-shared").await;
    assert_eq!(status, StatusCode::NOT_FOUND);
    let (status, _) = oneshot_as(routes(ctx.clone()), "GET", "/alice-job", "alice", "org-shared").await;
    assert_eq!(status, StatusCode::OK);

    let (status, _) = oneshot_as(
        routes(ctx.clone()),
        "DELETE",
        "/alice-job",
        "bob",
    ...
    let wrong_node = FinetuneTenantContext {
        owner_user_id: Some("alice".to_owned()),
        org_id: Some("org-shared".to_owned()),
        node_id: "other-node".to_owned(),
    };
    assert!(store.get_for(&wrong_node, "alice-job").await.is_err());
}
...
async fn router_wrappers_route_and_respond() {
    ...
    // DELETE /:id unknown → 404 before any worker request.
    ...
    // GET /:id/stream unknown → 404 before any worker request.
