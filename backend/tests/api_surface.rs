...
async fn dispatch_remote_maps_node_error_to_bad_gateway() {
    let node = spawn(Router::new().route(
        "/api/finetune/start",
        post(|| async { (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({ "error": "no gpu" }))) }),
    ))
    .await;
    let ctx = ctx_for(temp_store(), DEAD_URL);
    let body = json!({
    ...
    assert!(err["error"].as_str().unwrap().contains("remote node returned"));
...
async fn merge_value_worker_error_is_bad_gateway() {
    let worker = spawn(Router::new().route(
        "/finetune/merge",
        post(|| async { (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({ "error": "merge blew up" }))) }),
    ))
    .await;
    let ctx = ctx_for(temp_store(), worker);
...
async fn stream_local_worker_success_is_event_stream() {
    let worker = spawn(Router::new().route("/finetune/:id/stream", get(|| async { sse_ok() }))).await;
    let ctx = ctx_for(temp_store(), worker);
...
async fn oneshot(router: Router, method: &str, uri: &str, body: Option<Value>) -> (StatusCode, Value) {
